...
/*	positive		   negative		0		*/ 
/************************************************************************/
VirtScribbler::VirtScribbler(ServerSerial &port, char const fileName[]) : GameEngine(), mPort(&port)
{
	int	row, col;
	...
	strcpy(mName, "Virt Scribbler");
	memset(mPass, ' ', 16);
	memset(IPRE_data, 127, 4);
