...
int	Scribbler::getScribblerName(char name[16])
{
    	if (!sendScribblerCommand(GET_NAME1, name))
		return 0;

        if (!getFinalAck(GET_NAME1))
		return 0;

      	if (!sendScribblerCommand(GET_NAME2, name + 8))
		return 0;

        return getFinalAck(GET_NAME2);
}
...
{
     	if (!sendScribblerCommand(GET_PASS1, pass))
		return 0;

        if (!getFinalAck(GET_PASS1))
		return 0;

      	if (!sendScribblerCommand(GET_PASS2, pass + 8))
		return 0;

        return getFinalAck(GET_PASS2);
}
...
/* writes the data bytes						*/
/************************************************************************/
int	Scribbler::getScribblerData(char data[8])
{
      	if (!sendScribblerCommand(GET_DATA, data))
		return 0;

        return getFinalAck(GET_DATA);
}
/****************************************************************/
