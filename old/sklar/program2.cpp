/**
 * program2.cpp (19jan09/sklar+cummins)
 *
 *
 *
 **/
...
int main( int argc, char **argv ) {
  ...
  printf( "\nhello!\n" );
  printf( "this program should make the robot obey a few commands, entered by you!.\n" );
  ...
  printf( "\nbelow, i'll display a 'prompt', which looks like this '>>>'.\n" );
  printf( "at the prompt, you can enter one of the following commands:\n" );
  printf( "if you want to tell the robot to make a sound, then type 'beep'\n" );
  printf( "if you want to tell the robot to move forward, then type 'forward'\n" );
  printf( "if you want to tell the robot to move backward, then type 'backward'\n" );
  printf( "if you want to tell the robot to stop moving, then type 'stop'\n" );
  printf( "if you want to exit the program, then type 'quit'\n" );
  printf( "make sure to use lower case when you enter commands!\n" ); 
  printf( "okay, are you ready? here comes the prompt...\n\n" );

  while( more ) {
    printf( ">>> " );
    scanf( "%s", usercmd );
    if ( ! strcmp( usercmd, "beep" )) {
      printf( "now i'm telling the robot to 'beep'.\n" );
      beep( robot, 440, 1 );
    }
    else if ( ! strcmp( usercmd, "forward" )) {
      printf( "now i'm telling the robot to move forward.\n" );
      drive( robot, 200, 200, 1000 );
    }
    else if ( ! strcmp( usercmd, "backward" )) {
      printf( "now i'm telling the robot to move backward.\n" );
      drive( robot, 0, 0, 1000 );
    }
    else if ( ! strcmp( usercmd, "turn" )) {
      printf( "now i'm telling the robot to turn.\n" );
      drive( robot, 200, 10, 1000 );
    }
    else if ( ! strcmp( usercmd, "stop" )) {
      printf( "now i'm telling the robot to stop moving.\n" );
      stop( robot );
    }
    else if ( ! strcmp( usercmd, "quit" )) {
      more = false;
    }
    else {
      printf( "i don't understand the command that you typed. please try again.\n" );
    }
  }
